...
app.use(function verifyToken (req, res, next) {
  if (!config.tokens.length) return next()
  if (~config.tokens.indexOf(req.body.token)) return next()
  res.status(403).end()
})

app.post('/', function (req, res, next) {
  if (!/^(?:iou|kari)$/i.test(req.body.text)) return next()
...
app.post('/', function (req, res, next) {
  if (!/^(?:iou|kari)\s+history$/i.test(req.body.text)) return next()
...
app.post('/', function (req, res, next) {
  var match = req.body.text.match(/^(?:iou|kari)\s+<@([0-9A-Z]+)>$/i)
...
app.post('/', function (req, res, next) {
  var match = req.body.text.match(/^(?:iou|kari)\s+<@([0-9A-Z]+)>\s+[\$€]?([0-9]+)(?:\s+(.+))?$/i)
  ...
  balance.issue(debitor, creditor, amount, subject)
    .then((note) => {
      var text = `<@${note.debitor}> が <@${note.creditor}> から ${note.amount} 円借りました。`
                 + '\n' + getBalanceBetween(debitor, creditor)
      log(text)
...
app.post('/', function (req, res, next) {
  var match = req.body.text.match(/^(?:ask|kashi)\s+<@([0-9A-Z]+)>\s+[\$€]?([0-9]+)(?:\s+(.+))?$/i)
  ...
  balance.issue(debitor, creditor, amount, subject)
    .then((note) => {
      var text = [
        `<@${note.creditor}> が <@${note.debitor}> に ${note.amount} 円貸しました。`,
        getBalanceBetween(debitor, creditor)
      ]
...
app.post('/', function (req, res, next) {
  ...
  if (!/^(?:iou|kari)\s+help$/i.test(req.body.text)) {
    text.push('I couldn\'t understand that :thinking_face:. Here\'s my entire language:')
  }

  text.push(
    '```',
    '(iou|kari)              # Shows all your debts',
    '(iou|kari) @user        # Get a list of your accounts with @user',
    '(iou|kari) @user $20    # Add $20 to your debt to @user (the "$" is optional)',
    '(ask|kashi) @user $20   # Make @user owe you $20',
    '(iou|kari) help         # This',
    '(iou|kari) history      # Shows a list of all the transactions you where involved',
    '```'
  )
...
function getBalanceBetween (debitor, creditor) {
  ...
  if (amount === 0) {
    return `<@${debitor}> と <@${creditor}> は貸し借りありません。`
  } else if (amount > 0) {
    return `<@${debitor}> は <@${creditor}> に ${amount} 円借りています。`
  } else {
    return `<@${creditor}> は <@${debitor}> に ${-amount} 円借りています。`
  }
}
